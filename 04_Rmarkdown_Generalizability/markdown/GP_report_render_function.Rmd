...
NameMonth = monthNames[as.numeric(params$month)]

settings = PrescRiptions::setConfig(rootdir = "./output",
                                    year = params$year, month = params$month, region = "England")

dataAll = PrescRiptions::monthlyData_import(settings, sample = TRUE)
...
dt <- get(settings$plpdName) %>%
  select(PRACTICE, ITEMS, NIC, ACT.COST, QUANTITY) 
dt_stat <- data.table(sapply(dt[,c(2:5)], summary), keep.rownames = TRUE)
...
# ATTENTION: couldn't use the parameter in this code chunk. Had to specify the practice code.
...
n <- 5
dt_NIC_rate_top5 <- dt_NIC_rate[dt_NIC_rate$NIC_rate > quantile(dt_NIC_rate$NIC_rate,prob=1-n/100),]
